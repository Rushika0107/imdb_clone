...
const Navbar = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      navigate(`/movies?search=${encodeURIComponent(searchQuery)}`);
      setSearchQuery('');
      setIsMenuOpen(false); // Close menu on search (mobile view)
    }
  };
  ...
  const navItems = [
    ...
    { label: 'Top Rated', path: '/top-rated' },
    { label: 'Coming Soon', path: '/coming-soon' },
    { label: 'Actors', path: '/actors' }, // Added Actors page link
    { label: 'Favorites', path: '/favorite-actors' }, // Added Favorites page link
  ];
